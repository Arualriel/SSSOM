...
global m,S,at,eb,en,beta,c,Nmax,lp,tmax,push_rate,noclass,age_wins,epsilon,s



### dados reais ###
def Dados():
   lista_de_dados=['data-prototipo-sssom/breast.arff','data-prototipo-sssom/diabetes.arff','data-prototipo-sssom/glass1.arff','data-prototipo-sssom/glass.arff','data-prototipo-sssom/pendigits.arff','data-prototipo-sssom/shape.arff','data-prototipo-sssom/vowel.arff','data-prototipo-sssom/liver.arff']
   dados=lista_de_dados[2]
   with open(dados,'r') as arquivo:
      linhas = arquivo.read().splitlines()
   with open(dados,'r') as arquivo:
      linha = arquivo.read()
   dimm=int(linhas[0])+1 # dimensao + rotulo
   dimS=len(linhas)-1 # quantidade de dados
   Dado=np.zeros((dimm,dimS))
   i=0
   j=0
   num=''
   aux=0
   for linha in linhas:
      if aux!=0:
         ...
         j=j+1
      aux=1
   return Dado,dimS,dimm, dados

def sorteiodados(Dado):
   k=rd.randint(0,S-1)
   x=Dado[:,k]
   return x
...
def achar_s2(C,x,cont,W,a_s,j,s1,s2):
   ...
   for z in range(Nmax):
      if((C[m-1,z]!=0) or (C[m-1,z]==x[m-1])):
         ...
         j=z
   N=cont
   if((ind2!=-1) and ((C[:,ind2]!=s1) and (a_s>=at))):
      s2=C[:,ind2]
...
def fase_de_convergencia(C,wins,W,conexao,x,a_s,s1,ind,j,N,delta,beta,Dado):
   remocoes=1  
   nmax=0 
   aux=0 
   while (remocoes!=0):
...
#### Convergence phase ####
W,C=fase_de_convergencia(C,wins,W,conexao,x,a_s,s1,ind,j,N,delta,beta,Dado)
###########################

#### global relevance ####
w=global_relevance(W,C,w)
##########################
with open(dados+'_pesos_w.txt','a') as arquivo:
   for i in range(m-1):
      for j in range(Nmax):
         arquivo.write(str(W[i,j])+' ')
      arquivo.write('\n')
print('w=',w)
print('W=',W)
print('C=',C)
